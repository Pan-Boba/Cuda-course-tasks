...
Выделение границ проведено с помощью библиотеки OpenCv (фильтр Canny). Настраевые гиперпараметры ддля выделения круга: N (кол-во первоначальных точек построения), K (кол-во итераций),  THRESHOLD_RADIUS (размер порога радиуса, используемый для оценки точности построения круга), THRESHOLD_COUNT (кол-во точек вблизи построенного круга, используется для оценки точности построения)
