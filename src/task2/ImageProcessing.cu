...
void secondTask::GuassianBlur2D()
{
	/*cv::Mat3b image = cv::imread("ImageSample.png");
	cv::Mat3b result = img.clone();
	
	ConvoluteWithKernel(image.data, result.data, img.cols, img.rows);
